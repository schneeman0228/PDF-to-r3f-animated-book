...
// PDFページをテクスチャに変換する関数
const pdfPageToTexture = async (pdfPage, scale = 2) => {
...
const easingFactorFold = 0.3; // Controls the speed of the easing 開き具合
...
/*
pages.forEach((page) => {
  useTexture.preload(`/textures/cover.pdf`);
  useTexture.preload(`/textures/book-cover-roughness.jpg`);
});
*/

// ページコンポーネント
const Page = ({ number, textures, page, opened, bookClosed, ...props }) => {
  ...
  const manualSkinnedMesh = useMemo(() => {
    ...
    // 表裏のマテリアル設定
    const materials = [
      ...pageMaterials,// 側面用の基本マテリアル
      // 右ページ（表）
      new MeshStandardMaterial({
        color: whiteColor,
        map: textures[number*2],
        roughness: 1.5,
...
// メインのブックコンポーネント
//cover_pdfUrl
export const PDFBook = ({ pdfUrl, ...props }) => {
  const [page] = useAtom(pageAtom);
  const [delayedPage, setDelayedPage] = useState(page);
  
  //const { textures_cover, cover_loading, cover_error } = usePDFPages(cover_pdfUrl);
  ...
  const pageCount = Math.floor((textures.length + 1) / 2);
